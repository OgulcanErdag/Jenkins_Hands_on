...
- Trigger Jenkins jobs with a webhook
...
- Copy the Jenkins URL from the AWS Management Console, paste it into `Payload URL` field, add `/github-webhook/` at the end of the URL, and click on `Add webhook`.

```text
http://ec2-54-144-151-76.compute-1.amazonaws.com:8080/github-webhook/
```

- Change the Python code in your local repository to print `Hello World for Jenkins Job` and save.
...
- Observe the newly built triggered with the `git push` command under `Build History` on the Jenkins project page.
...
- Create a Java file on the `jenkins-pipeline-project` local repository, name it `Hello.java`, add coding to print `Hello from Java`, and save.
...
- Since the Jenkins Server is running on the Java platform, we can leverage the already available Java environment.
...
- Observe the newly built triggered with `git push` command on the Jenkins project page.
