...
The purpose of this hands-on training is to learn how to configure an agent node for a Jenkins Server. 
...
- Launch an instance on Amazon Linux 2023 AMI with a security group allowing SSH (port 22) as the agent node.
...
- Install  `Command Agent Launcher` Plugin on the Jenkins controller.
...
- Open the "authorized_keys" file with an editor and paste the code that you copied from the public key(id_rsa.pub). Save the "authorized_keys" file.

- Get the agent node private IP and go to the Jenkins Controller server and test the SSH connection.
...
- **The second one is to copy agent file from the Jenkins server to the agent node.**

- Go to the "/root" folder on the agent node instance. Create a folder under "/root" and name it "bin". Get the agent file from the Jenkins Controller server.
...
- Select `Launch agent via execution of command on the controller` from the dropdown menu in the ``Launch method`` field.

- Enter `ssh -i /var/lib/jenkins/.ssh/<the_key_file> -o StrictHostKeyChecking=no root@<agent_ip> java -jar -Djava.io.tmpdir=/mnt /root/bin/agent.jar` in the ``Launch command`` field.

- Select `Keep this agent online as much as possible` from the dropdown menu in the ``Availability`` field.

- Click `Save`.

- Check the console logs in case of failure to launch the agent node. If there is a approvement issue, go to `Manage Jenkins`,  select the `In-process Script Approval`, and `approve` the script.

- Go to the Jenkins dashboard. Check the controller and agent nodes on the left hand menu. 
...
- Enter `agent-job`, then select `free style project` and click `OK`.
  ...
  - Write down just ``echo "Hello World, This is a job for agent node"` to execute the shell command, in the text area shown.
...
- Check the "Build Queue" section to see the build process running on the agent node on the left-hand menu. 
...
- Go to the dashboard and create a `Seed Job` in the form of `Free Style Project`. To do so;
...
- Observe that the DSL Job was created.

- Go to the `First-Maven-Project-Via-DSL` job.

- Select `Configure`, in the `Build` section, set `Maven Version` to a defined/valid one.
...
- Connect to the Jenkins server with SSH.
    ...
    Definition:
        Pipeline script from SCM:
            Git:
                ...
                Branches to build: It must be the same branch name with your `jenkins-first-webhook-project` Github repository. If your repository's default branch name is "main", then change "master" to "main".
